...
namespace ServerSocket.Actions.GetHand
{
    ...
    public GetHandAction() : base(
        ...
        protected override BaseSerializer PerformAction(ClientConnection conn, BaseSerializer requestData)
        {
            ...
            GameManagerLib.Models.Card[] hand = new GameManagerLib.Models.Card[13];

            for (int i = 0; i < hand.Length; i++) {
                hand[i] = new GameManagerLib.Models.Card((CardFigure)cards[i].Figure, (CardColor)cards[i].Color, (PlayerTag)data.PlayerID, (CardState)cards[i].State);
            }
